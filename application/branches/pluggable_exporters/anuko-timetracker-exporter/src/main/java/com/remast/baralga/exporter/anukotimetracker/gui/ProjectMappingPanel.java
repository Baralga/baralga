...
/**
 * A panel which helps matching the Baralga projects to
...
public class ProjectMappingPanel extends JXPanel {
    ...
    private void initialize() {
        ...
        for( Project p : getBaralgaProjects() ) {
            ...
            this.add(getProjectSelector(info.getActivities()), "3, "+row); //$NON-NLS-1$
            row = row+2;
        }
    }
    ...
    private SortedSet<Project> getBaralgaProjects() {
        if( filter == null ) {
            return new TreeSet<Project>(this.baralgaData.getProjects());
        } else {
    ...
    private static final AnukoActivity ALL_PROJECTS_DUMMY = new AnukoActivity(-4711, "*"); //$NON-NLS-1$

    private static final LabeledItem<AnukoActivity> ALL_PROJECTS_FILTER_ITEM = new LabeledItem<AnukoActivity>(ALL_PROJECTS_DUMMY, ""); //$NON-NLS-1$
    
    
    private static JComboBox getProjectSelector(List<AnukoActivity> projects) {
        
        EventList<LabeledItem<AnukoActivity>> eventList =
            new BasicEventList<LabeledItem<AnukoActivity>>();
        eventList.add(ALL_PROJECTS_FILTER_ITEM);
        for( AnukoActivity p : projects ) {
            eventList.add( new LabeledItem<AnukoActivity>(p, p.getName()) );
        }
        ...
        //projectFilterSelector.setToolTipText("Huhu!"); //$NON-NLS-1$
